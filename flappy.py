...
def mainGame(movementInfo):
    ...
    #  set yolo to detect the objects
    yolo=demo.YOLO()
    ...
    while True:
        ...
        # detect bird and the first gap that the bird will go through
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            box = out_boxes[i]
            score_detect = out_scores[i]

            label = '{} {:.2f}'.format(predicted_class,score_detect )
            ...
            cor_x=(left+right)/2
            cor_y=(bottom+top)/2
            cor_y_pipe=top+(bottom-top)*lamda
            # calculate positions of the bird and the pipe
            if(predicted_class=='bird'):
                bird_cor[0]=cor_x
                bird_cor[1]=cor_y
            else:
                if(right>SCREENWIDTH * 0.2):
                   if(pipe_cor[0]==0):
                       pipe_cor[0]=cor_x
                       pipe_cor[1]=cor_y_pipe
                       print('bottom')
                       print(bottom)
                   elif(pipe_cor[0]>cor_x):
                       pipe_cor[0]=cor_x
                       pipe_cor[1]=cor_y_pipe
                       print('bottom')
                       print(bottom)
            pygame.draw.rect(SCREEN,[255,0,0],[left,top,(right-left),(bottom-top)],3)
            text_surface = my_font.render(label, False, (0,0,0))
            SCREEN.blit(text_surface,(left,top-10))
        print ('bird')
        print (bird_cor)
        print ('pipe')
        print (pipe_cor)
        # when bird's height is lower than pipe_cor[1]
        if(pipe_cor[1]!=0):
            if(bird_cor[1]>pipe_cor[1]):
                playerVelY = playerFlapAcc
                playerFlapped = True
        elif(bird_cor[1]>SCREENHEIGHT*0.4 and bird_cor[1]!=0):
            playerVelY = playerFlapAcc
            playerFlapped = True
            print('jump!!')
        pygame.display.update()
